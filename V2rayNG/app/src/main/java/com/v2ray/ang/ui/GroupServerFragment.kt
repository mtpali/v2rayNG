...
class GroupServerFragment : BaseFragment<FragmentGroupServerBinding>(),
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.refreshLayout.isEnabled = subId.isNotEmpty()
        binding.refreshLayout.setOnRefreshListener(this)
        binding.refreshLayout.setDistanceToTriggerSync(
            (160 * resources.displayMetrics.density).toInt()
        )

        mainViewModel.isUpdatingSubscriptions.observe(viewLifecycleOwner) { isRefreshing ->
            binding.refreshLayout.isRefreshing = isRefreshing && mainViewModel.subscriptionId == subId
        }
    ...
    override fun onRefresh() {
        if (subId.isEmpty() || !ownerActivity.importConfigViaSub()) {
