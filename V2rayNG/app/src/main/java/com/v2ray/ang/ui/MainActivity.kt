...
class MainActivity : HelperBaseActivity(), NavigationView.OnNavigationItemSelectedListener {
    ...
    private fun setupNavigationDrawer() {
        ...
        lifecycleScope.launch {
            repeatOnLifecycle(Lifecycle.State.STARTED) {
                mainViewModel.lanSharing.collect { enabled ->
                    binding.navView.menu.findItem(R.id.lan_sharing).isChecked = enabled
                }
            }
        }
    ...
    override fun onOptionsItemSelected(item: MenuItem) = when (item.itemId) {
        R.id.clear_traffic -> {
            AlertDialog.Builder(this).setMessage(R.string.confirm_clear_traffic)
                .setPositiveButton(android.R.string.ok) { _, _ -> mainViewModel.clearTraffic() }
    ...
    fun importConfigViaSub(): Boolean {
        val update = mainViewModel.beginSubscriptionUpdate() ?: return false
        showLoading()
    ...
    private fun showRenameConfigsDialog() {
        val groupId = mainViewModel.subscriptionId
        if (groupId.isBlank()) {
        ...
        val input = EditText(this).apply {
            hint = getString(R.string.hint_profile_name_prefix)
            isSingleLine = true
        }
        AlertDialog.Builder(this)
            .setTitle(R.string.title_rename_configs)
            .setView(input)
            .setPositiveButton(android.R.string.ok) { _, _ ->
                val prefix = input.text?.toString().orEmpty().trim()
                if (prefix.isEmpty()) {
                ...
                lifecycleScope.launch {
                    try {
                        val count = mainViewModel.renameConfigs(groupId, prefix).await()
                        mainViewModel.reloadServerList()
                        refreshGroupTabTitles()
                        toast(getString(R.string.toast_profiles_renamed, count))
                    } catch (cancelled: kotlinx.coroutines.CancellationException) {
                        throw cancelled
                    } catch (error: Exception) {
