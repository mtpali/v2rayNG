...
class MainViewModel(application: Application) : AndroidViewModel(application) {
    ...
    private val updatingSubscriptions = MutableLiveData(false)
    val isUpdatingSubscriptions: LiveData<Boolean> = updatingSubscriptions
     ...
     */
    fun beginSubscriptionUpdate(): Deferred<SubscriptionUpdateResult>? {
        if (!subscriptionUpdateGate.tryBegin()) return null
            ...
            try {
                withContext(Dispatchers.IO) {
                    if (groupId.isEmpty()) {
                        ...
                        val sub = MmkvManager.decodeSubscription(groupId)
                        if (sub == null) SubscriptionUpdateResult()
                        else AngConfigManager.updateConfigViaSub(SubscriptionCache(groupId, sub))
                    }
                }
            } finally {
                subscriptionUpdateGate.finish()
                updatingSubscriptions.value = false
            }
        }
    }

    fun renameConfigs(groupId: String, prefix: String): Deferred<Int> =
        viewModelScope.async(Dispatchers.IO) {
            MmkvManager.renameServerProfiles(MmkvManager.decodeServerList(groupId), prefix)
        }
